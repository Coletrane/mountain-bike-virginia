  ...
  for (const file of imageFiles) {
    ...
    if (whitelist(file)) {
      it(`${file} should have 480 width` , () => {
        expect(imageFiles.find(imgFile => {
          return `${file.replace(path.extname(file), '')}-480${path.extname(file)}` === imgFile
        }))
          .not.to.be.undefined
      })
      it(`${file} should have 720 width` , () => {
        expect(imageFiles.find(imgFile => {
          return `${file.replace(path.extname(file), '')}-720${path.extname(file)}` === imgFile
        }))
          .not.to.be.undefined
      })
    }
