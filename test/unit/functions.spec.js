...
let head = headTags("title", "description", "keywords", {
...
describe("headTags tests", () => {
  it("should have a title tag", () => {
    expect(head.title).to.equal("title")
  })
  ...
  describe("meta tags test", () => {
    head.meta.forEach((meta, i, arr) => {
      ...
      if (meta.name === "description") {
        it("should have description", () => {
          expect(content).to.equal("description")
        })
      }

      if (meta.name === "keywords") {
        it("should have keywords", () => {
          expect(content).to.equal("keywords")
        })
      }
    ...
    describe("child route headTags test", () => {
      let childHeadTags = headTags("title", "description", "keywords", {
        imgRoute: "foliage.jpg",
        route: "results-middle-mountain-momma-2018"
      })

      it("should have the correct og:image", () => {
        let ogImage = childHeadTags.meta.filter(
          tag => tag.property === "og:image"
        )[0]

        expect(ogImage.content.endsWith(".jpg")).to.be.true
      })
    })
  })
})
