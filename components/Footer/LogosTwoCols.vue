...
export default {
  ...
  computed: {
    sponsorGroups() {
      ...
      let starlightIndex = this.sponsors.findIndex(
        sponsor => sponsor.class === "starlight"
      )
      let jtrgIndex = this.sponsors.findIndex(
        sponsor => sponsor.class === "jtrg"
      )
      let temp = this.sponsors[jtrgIndex]
      this.sponsors[jtrgIndex] = this.sponsors[starlightIndex]
      this.sponsors[starlightIndex] = temp

      starlightIndex = this.sponsors.findIndex(
        sponsor => sponsor.class === "starlight"
      )
      let bathIndex = this.sponsors.findIndex(
        sponsor => sponsor.class === "bath"
      )
      temp = this.sponsors[bathIndex]
      this.sponsors[bathIndex] = this.sponsors[starlightIndex]
      this.sponsors[starlightIndex] = temp

      let blackwaterIndex = this.sponsors.findIndex(
        sponsor => sponsor.class === "blackwater"
      )
      let woodsIndex = this.sponsors.findIndex(
        sponsor => sponsor.class === "woods"
      )
      temp = this.sponsors[woodsIndex]
      this.sponsors[woodsIndex] = this.sponsors[blackwaterIndex]
      this.sponsors[blackwaterIndex] = temp

      let eastCoastersIndex = this.sponsors.findIndex(
        sponsor => sponsor.class === "east-coasters"
      )
      ...
      this.sponsors.forEach((elem, i, arr) => {
        if (i % 2 === 0) {
          groups.push([this.sponsors[i], this.sponsors[i + 1]])
        }
      })
